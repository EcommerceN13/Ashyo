...
import { AppleStore, FaceBookIcon, PlayMarket, TelegramIcon, TwitterIcon, YouTubeIcon } from '@/icons'
        ...
        <div className='containers flex justify-between'>
            <div>
                ...
                <div className='flex items-center gap-[10px] mt-[21px]'>
                    {socialList.map(item => (
                        <Link className='bg-[#EBEFF3] rounded-[7px] w-[44px] h-[40px] flex items-center justify-center' href={item.path} key={item.id}>{item.icon}</Link>
                    ))}
                </div>
            ...
            <div>
                ...
                <div className='space-y-[12px] flex flex-col'>
                    {menuList.map(item => <Link className='text-[16px] leading-[17.66px] text-[#8c8989] opacity-70' key={item.id} href={item.path}>{item.title}</Link>)}
                </div>
            </div>
            <div className='flex flex-col'>
                ...
                <Input extraClass='!w-[314px]' placeholder="O'zingiz qiziqtirgan savollarni bering" type='text'></Input>
            </div>
        </div>
    </footer>
  )
