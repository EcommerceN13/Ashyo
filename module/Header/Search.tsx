...
import { getCategorySearch } from '@/service/SearchCategory'
...
const Search = () => {
  ...
  return (
    <form className='w-[518px] relative'>
            <Input onBlur={() => setTimeout(() => setSearchValue("empty"), 500)} onChange={handleSearchChange} extraClass={`!py-[15px] !px-[21px] !text-[10px] sm:!py-[17px] sm:!px-[20px] sm:!text-[14px]`} type='text' placeholder="What are you looking for?" />
            ...
            <ul className={`${searchValue == "empty" && "h-0 opacity-0"} z-50 duration-300 overflow-hidden ${isLoading && "px-5"} absolute w-full bg-white top-[67px] pt-[39px] pb-[10px] shadow-md`}>
